...
<head>
  <style>
    ...
    .svg-w {
      ...
      max-width: 95vw;
      max-height: 95vh;
      filter: drop-shadow(0 0 35px rgba(93,214,44,0.15));
    }
...
<body>
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-[-10%] left-[15%] w-[600px] h-[600px] glow-ambient pointer-events-none z-0 filter blur-3xl opacity-90" />
      <div className="absolute bottom-[-15%] right-[10%] w-[700px] h-[700px] glow-ambient pointer-events-none z-0 filter blur-3xl opacity-60" />
        ...
        {isDragging && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 15 }}
              className="bg-brand-card border border-brand-accent/30 rounded-3xl p-8 max-w-md text-center shadow-[0_0_40px_rgba(93,214,44,0.15)] relative overflow-hidden pointer-events-none"
            >
      ...
      <header className="fixed top-6 left-1/2 -translate-x-1/2 z-40 max-w-5xl w-[calc(100%-2.5rem)] bg-brand-card/90 backdrop-blur-md rounded-full border border-brand-accent/20 shadow-[0_0_35px_rgba(93,214,44,0.08)] px-4 h-[58px] flex items-center justify-between text-brand-text select-none">
        ...
        <div className="flex items-center gap-2">
           ...
           <button
              ...
              className="flex items-center gap-1.5 bg-[#0f0f0f] hover:bg-brand-bg/85 border border-brand-accent/30 hover:border-brand-accent/60 text-brand-text transition-all px-3.5 py-1.5 rounded-full text-xs font-bold active:scale-95 shadow-sm shadow-brand-accent/15 cursor-pointer h-9"
        ...
        <div className="bg-brand-card/95 backdrop-blur-md rounded-full shadow-[0_12px_45px_rgba(93,214,44,0.12)] border border-brand-accent/25 px-2 py-1.5 flex justify-between items-center h-14 relative w-full">
          {[
            ...
            >
              ...
              <span>{tab.label}</span>
              {tab.isNew && (
                <span className="absolute -top-1 -right-0.5 w-2.5 h-2.5 rounded-full bg-brand-accent shadow-[0_0_8px_#5dd62c] animate-ping" />
              )}
            </button>
          ))}
        </div>
