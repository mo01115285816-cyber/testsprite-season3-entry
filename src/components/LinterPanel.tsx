...
export default function LinterPanel({
 ...
 <span className="font-sans font-black text-xs text-brand-text">{t('linter.header')}</span>
 </div>
 <button
 ...
 >
 <span>{t('linter.realtimeTab')} ({lintIssues.length})</span>
 ...
 >
 <span>{t('linter.aiTab')} ({deepIssues.length})</span>
 {activeLintTab === 'deep' && (
 ...
 <h4 className="text-xs font-extrabold text-zinc-100 mb-1">{t('linter.emptyTitle')}</h4>
 <p className="text-[10px] text-zinc-400 leading-relaxed">{t('linter.emptyDesc')}</p>
 </div>
 ) : (
 ...
 <div className="bg-brand-card p-4 rounded-2xl border border-brand-accent/15 mb-2 shadow-none">
 <h4 className="text-xs font-black text-zinc-200 mb-1 flex items-center gap-1.5">
 <Sparkles className="w-3.5 h-3.5 text-brand-accent animate-pulse" />
 <span>{t('linter.deepTitle')}</span>
 </h4>
 ...
 className="w-full py-2.5 px-3 rounded-xl bg-gradient-to-r from-brand-accent via-brand-deep to-brand-accent text-brand-bg transition-all text-xs font-extrabold flex items-center justify-center gap-1.5 shadow-none active:scale-95 cursor-pointer disabled:opacity-40"
 >
 {isDeepLinting ? (
 ...
 <span>{isDeepLinting ? t('linter.deepAnalyzing') : t('linter.startDeepScan')}</span>
 </button>
 </div>
 ...
 <h4 className="text-[11px] font-extrabold text-zinc-300">{t('linter.aiReading')}</h4>
 <p className="text-[9px] text-zinc-400 max-w-xs mt-1">{t('linter.aiReadingDesc')}</p>
 </div>
 ) : (
 <>
 {deepIssues.length === 0 ? (
 ...
 <h5 className="text-[11px] font-bold text-zinc-400 mt-2">{t('linter.noDeepReviews')}</h5>
 <p className="text-[9px] text-zinc-650 mt-1">{t('linter.clickToRecord')}</p>
 </div>
 ) : (
 <div className="flex flex-col gap-2.5">
 {deepLintSummary && (
 ...
 onClick={() => applyQuickFix(issue.targetText!, issue.replacementText!)}
 className="w-full mt-2.5 py-1.5 px-2.5 rounded-lg bg-brand-accent/10 hover:bg-brand-accent text-brand-accent hover:text-brand-bg transition-all text-[10px] font-black border border-brand-accent/25 flex items-center justify-center gap-1 cursor-pointer"
 title={t('linter.quickFixShort')}
 >
 <Wrench className="w-3 h-3" />
 <span>{t('linter.quickFixShort')}</span>
 </button>
 )}
 </div>
 </div>
